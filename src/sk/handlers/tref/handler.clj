...
(defn get-entrenadores
  "Gets all entrenadores from database :ex: (get-entrenadores)"
  []
...
(defn get-estadios
  "Gets all estadios from database :ex: (get-estadios)"
  []
...
(defn get-entrenador-nombre
  ...
  [id]
  (let [row (first (Query db ["SELECT
                       CONCAT(nombre,' ',paterno,' ',materno) as nombre
                       FROM entrenadores
                       WHERE id = ?" id]))]
    (:nombre row)))
;; End get-entrenador-nombre
...
(defn get-equipos
  "Gets all equipos from database :ex: (get-equipos)"
  []
